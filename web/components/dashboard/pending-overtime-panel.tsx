...
import { Check, X, Loader2, Timer } from 'lucide-react'
...
import { useOvertimes, useApproveOvertime, useRejectOvertime } from '@/hooks/use-overtime'
...
export function PendingOvertimePanel({ limit = 6 }: { limit?: number }) {
  const { data, isLoading } = useOvertimes({ status: 'pending', per_page: limit })
      ...
      <div className="rounded-xl border bg-card">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm">Pending Overtime Requests</h3>
            {total > 0 && (
              <Badge className="bg-amber-100 text-amber-700 border-amber-200">{total}</Badge>
            )}
          </div>
          <Link href="/admin/overtime" className="text-xs text-primary hover:underline">
            View all
          </Link>
        </div>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    {fmtDate(req.date)} · {req.overtime_hours}h · {TYPE_LABELS[req.overtime_type]}
                  </p>
                </div>
              </div>
